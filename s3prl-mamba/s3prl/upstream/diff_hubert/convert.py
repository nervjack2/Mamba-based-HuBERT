...
import s3prl
from s3prl.upstream.diff_hubert.hubert_model import (
    HubertConfig,
    HubertModel,
...
def load_converted_model(ckpt: str):
    ...
    for required_key in [
        ...
        if required_key not in ckpt_state:
            ...
            try:
                if required_key == "task_cfg":
                    ckpt_state["task_cfg"] = ckpt_state["cfg"]["task"]
                elif required_key == "model_cfg":
                    ckpt_state["model_cfg"] = ckpt_state["cfg"]["model"]
                elif required_key == "model_weight":
                    ckpt_state["model_weight"] = ckpt_state["model"]
                else:
                    ckpt_state["dictionaries_symbols"] = ckpt_state["task_state"]["dictionaries"]
