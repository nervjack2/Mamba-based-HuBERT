...
from ..diff_wav2vec2.wav2vec2_model import (
...
class HubertConfig:
    ...
    # Mambaformer
    mamba_ssm_state_expand: int = field(
        default=16,
        metadata={"help": "state expansion form ssm in mambaformer layer"},
    )
    mamba_conv_kernel_size: int = field(
        default=4,
        metadata={"help": "conv-kernel-size for convolution in mambaformer layer"},
    )
    mamba_block_expand: int = field(
        default=2,
        metadata={"help": "block expansion in mambaformer"},
    )
    ...
    causal_attn: bool = field(
        default=False,
        metadata={"help": "whether to use causal attention instead of normal attention"}
    )
...
class HubertModel(torch.nn.Module):
    def __init__(
        ...
        if cfg.layer_type == "mambaformer" or cfg.layer_type == "bimambaformer" or cfg.layer_type == "mamba2former":
            encoder_cls = MambaformerEncoder
        else:
    ...
    def extract_features(
        ...
        feature = res["x"]
        return feature, res["padding_mask"], res["features"], res["layer_results"]
