...
# from mamba_ssm.modules.mamba_simple import Mamba, BiMamba
...
LAYER_TYPE_CHOICES = ChoiceEnum(["transformer", "conformer", "mambaformer", "bimambaformer", "mamba2former"])
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.causal_attn = args.causal_attn
    ...
    def extract_features(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.layerdrop):
                x, (z, lr) = layer(
                    x, self_attn_padding_mask=padding_mask, need_weights=False, self_attn_mask=attn_mask,
                )
                if i >= min_layer:
...
class MambaformerEncoder(nn.Module):
    def build_encoder_layer(self, args: Wav2Vec2Config, **kwargs):
        if args.layer_type == "bimambaformer":
            layer = BiMambaformerSentenceEncoderLayer(
                embedding_dim=self.embedding_dim,
                ffn_embedding_dim=args.encoder_ffn_embed_dim,
                ssm_state_expand=args.mamba_ssm_state_expand,
                conv_kernel_size=args.mamba_conv_kernel_size,
                block_expand=args.mamba_block_expand,
                dropout=self.dropout,
                activation_dropout=args.activation_dropout,
                activation_fn=args.activation_fn,
                layer_norm_first=args.layer_norm_first,
            )
        elif args.layer_type == "mamba2former":
            layer = Mamba2formerSentenceEncoderLayer(
                embedding_dim=self.embedding_dim,
                ffn_embedding_dim=args.encoder_ffn_embed_dim,
                ssm_state_expand=args.mamba_ssm_state_expand,
                conv_kernel_size=args.mamba_conv_kernel_size,
            ...
            layer = MambaformerSentenceEncoderLayer(
                embedding_dim=self.embedding_dim,
                ffn_embedding_dim=args.encoder_ffn_embed_dim,
                ssm_state_expand=args.mamba_ssm_state_expand,
                conv_kernel_size=args.mamba_conv_kernel_size,
    ...
    def extract_features(
        ...
        for i, layer in enumerate(self.layers):
            dropout_probability = np.random.random() if self.layerdrop > 0 else 1
            if not self.training or (dropout_probability > self.layerdrop):
                x, (z, lr, diff_lr) = layer(x)
                if i >= min_layer:
                    layer_results.append((x, z, lr, diff_lr))
                    # layer_results.append((x.transpose(0, 1), z, lr.transpose(0, 1)))
            if i == tgt_layer:
                r = x
                break
...
class MambaformerSentenceEncoderLayer(nn.Module):
    """
    Implements a Mambaformer Encoder Layer used in BERT/XLM style pre-trained
    models.
    """
    ...
    def __init__(
        ...
        # layer norm associated with the mamba layer
        self.mamba_layer_norm = LayerNorm(self.embedding_dim)

        if self.ffn_embedding_dim is not None and self.ffn_embedding_dim > 0:
            self.activation_fn = get_activation_fn(activation_fn)
            self.fc1 = nn.Linear(self.embedding_dim, ffn_embedding_dim)
            self.fc2 = nn.Linear(ffn_embedding_dim, self.embedding_dim)
    ...
    def forward(
        ...
        """
        LayerNorm is applied either before or after the mamba/ffn
        ...
        diff_layer_result = torch.diff(layer_result, dim=0)
        diff_layer_result = torch.cat([layer_result[0:1], diff_layer_result], dim=0)

        return x, (None, layer_result, diff_layer_result)
    

class BiMambaformerSentenceEncoderLayer(nn.Module):
    """
    Implements a BiMambaformer Encoder Layer used in BERT/XLM style pre-trained
    models.
    """
    ...
    def __init__(
        ...
        # layer norm associated with the mamba layer
        self.bimamba_layer_norm = LayerNorm(self.embedding_dim)
    ...
    def forward(
        ...
        if self.layer_norm_first:
            x = self.bimamba_layer_norm(x)
            # x = self.bimamba(x)
            
            x_rev = torch.flip(x, dims=(1,))
            x = self.mamba(x)
            x_rev = self.mamba_rev(x_rev)

            diff_x = torch.diff(x, dim=1)
            diff_x = torch.cat([x[:,0:1,:], diff_x], dim=1)
            diff_x_rev = torch.diff(x_rev, dim=1)
            diff_x_rev = torch.cat([x_rev[:,0:1,:], diff_x_rev], dim=1)

            x = x + torch.flip(x_rev, dims=(1, ))
            diff_x = diff_x + torch.flip(diff_x_rev, dims=(1, ))
        ...
        x = x.transpose(0, 1)
        layer_result = layer_result.transpose(0, 1)
        diff_x = diff_x.transpose(0, 1)

        return x, (None, layer_result, diff_x)


class Mamba2formerSentenceEncoderLayer(nn.Module):
    """
    Implements a Mamba2former Encoder Layer used in BERT/XLM style pre-trained
    models.
    """
