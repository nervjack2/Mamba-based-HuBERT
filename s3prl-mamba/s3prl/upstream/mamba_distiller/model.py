...
encoder_configs = {
    "tiny": { # ~9.2M
        "depth": 12, "embed_dim": 192
    },
    "small": { # ~23.2M
        "depth": 12, "embed_dim": 384
    },
    "medium": {
        "depth": 12, "embed_dim": 512
    },
    "base": {
...
class DistillerConfig:
    ...
    def __init__(self, config: dict):
        ...
        # Mamba encoder
        self.encoder_arch = str(config.get("encoder_arch", "base"))
        self.direction = str(config.get("direction", "bi"))
        self.mamba_type = str(config.get("mamba_type", "mamba"))
        self.ffn_dim = int(config.get("ffn_dim", 0))
        self.activation_fn = str(config.get("activation_fn", "relu"))
        self.dropout = float(config.get("dropout", 0.1))
        self.activation_dropout = float(config.get("activation_dropout", 0.1))
        self.ssm_cfg = dict(config.get("ssm_cfg", { "d_state":24, "d_conv":4, "expand":3}))
        self.residual_in_fp32 = bool(config.get("residual_in_fp32", True))
...
class DistillerModel(nn.Module):
    ...
    def __init__(self, config: DistillerConfig):
        ...
        self.config = config
        # Mamba encoder configurtion
        self.encoder_embed_dim = encoder_configs[config.encoder_arch]["embed_dim"]
        self.encoder_layers = encoder_configs[config.encoder_arch]["depth"] 

        # Convolutional feature extrator
        ...
        final_emb_size = self.encoder_embed_dim
        if self.task_emb_type == "add":
            self.task_embedding = nn.Embedding(config.n_tasks, self.encoder_embed_dim)
            ...
            self.pred_layer_id = config.pred_layer_id
            assert self.n_tasks == len(self.pred_layer_id)
            assert self.n_tasks == self.encoder_layers + 1
        ...
        self.post_extract_proj = (
            nn.Linear(feat_emb_dim, self.encoder_embed_dim)
            if feat_emb_dim != self.encoder_embed_dim
            else None
        )
        ...
        if self.encoder_layers > 0:
            self.encoder = MambaEncoder(
                ...
                **encoder_configs[config.encoder_arch]
            )
        else:
