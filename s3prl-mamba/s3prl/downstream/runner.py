...
class Runner():
    ...
    def train(self):
        ...
        # grad scheduler 
        if self.config.get('grad_scheduler'):
            finetune_steps = int(self.config['grad_scheduler']['finetune_steps']*self.config['runner']['total_steps'])
            assert hasattr(self.upstream.model, 'set_require_grad')
            getattr(self.upstream.model, 'set_require_grad')(finetune_steps, pbar.n+1)
